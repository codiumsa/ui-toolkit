...
!function(e){e.module("qualitaCoreFrontend.config",[]).value("qualitaCoreFrontend.config",{debug:!0}),e.module("qualitaCoreFrontend.directives",[]),e.module("qualitaCoreFrontend.filters",[]),e.module("qualitaCoreFrontend.services",[]),e.module("qualitaCoreFrontend",["qualitaCoreFrontend.config","qualitaCoreFrontend.directives","qualitaCoreFrontend.filters","qualitaCoreFrontend.services","ngResource","ngCookies","ngSanitize","ngTouch","datatables","datatables.bootstrap","schemaForm","pascalprecht.translate","cgNotify","angular-underscore/filters","flow","ui.bootstrap","ui.select","ui.highlight","mgcrea.ngStrap.select","ncy-angular-breadcrumb","ui.router","oc.lazyLoad","ngStorage","LocalForageModule","datatables.buttons","datatables.colreorder","daterangepicker"])}(angular),angular.module("qualitaCoreFrontend").directive("fileupload",["$rootScope",function(e){return{template:'<div ng-show="form.uploadOptions.imageOnly"><div flow-init="{singleFile: true, target: form.uploadOptions.target}" flow-file-added="!!{png:1,gif:1,jpg:1,jpeg:1}[$file.getExtension()]"flow-files-submitted="form.uploader.flow.upload()"flow-files-added="filesAdded($files, $event, form.uploader.flow)"flow-name="form.uploader.flow"class="ng-scope"><h3>{{showTitle()}}</h3><div class="thumbnail" ng-show="form.uploader.currentFile && !form.uploader.flow.files.length"><img src="{{form.uploader.currentFile}}"/> </div><div class="thumbnail" ng-show="!form.uploader.flow.files.length && !form.uploader.currentFile"><img src="images/placeholder.png"></div><div class="thumbnail" ng-show="form.uploader.flow.files.length"><img flow-img="form.uploader.flow.files[0]"/> </div><div><span class="btn btn-primary" ng-show="!form.uploader.flow.files.length" flow-btn="">Seleccionar imagen<input type="file" sf-changed="form" ng-model="$$value$$" style="visibility: hidden; position: absolute;" /></span><span class="btn btn-info ng-hide" ng-show="form.uploader.flow.files.length" flow-btn="">Cambiar<input type="file" sf-changed="form" ng-model="$$value$$" style="visibility: hidden; position: absolute;" /></span><span class="btn btn-danger ng-hide" ng-show="form.uploader.flow.files.length" ng-click="form.uploader.flow.cancel()">Eliminar</span></div><p>Formatos permitidos: PNG, GIF, JPG y JPEG.</p></div></div><div ng-show="!form.uploadOptions.imageOnly"><div flow-init="{singleFile: true, target: form.uploadOptions.target}" flow-file-added="filesAdded($files, $event, uploader.flow)"flow-files-submitted="form.uploader.flow.upload()"flow-files-added="filesAdded($files, $event, form.uploader.flow)"flow-name="form.uploader.flow"class="ng-scope"><h3>{{showTitle()}}</h3><div class="drop" flow-drop ng-class="dropClass"><span class="btn btn-default" flow-btn>Cargar archivo<input type="file" ng-model="$$value$$" sf-changed="form" style="visibility: hidden; position: absolute;" /></span><b>OR</b>Arrastre el archivo aqu&iacute;</div><br/><div><div ng-repeat="file in form.uploader.flow.files" class="transfer-box">{{file.relativePath}} ({{file.size}}bytes)<div class="progress progress-striped" ng-class="{active: file.isUploading()}"><div class="progress-bar" role="progressbar"aria-valuenow="{{file.progress() * 100}}"aria-valuemin="0"aria-valuemax="100"NaN<span class="sr-only">{{file.progress()}}% Complete</span></div></div><div class="btn-group"><a class="btn btn-xs btn-danger" ng-click="file.cancel()">Cancelar</a><a class="btn btn-xs btn-info" ng-click="file.retry()" ng-show="file.error">Reintentar</a></div></div></div></div></div>',restrict:"E",tranclude:!0,scope:!0,link:function(t,n,o){t.uploader={},t.title=o.title,t.fileModel={},t.filesAdded=function(t,n,o){e.flow||(e.flow=o)}}}}]),angular.module("qualitaCoreFrontend").directive("offlineFormRecovery",function(e){return{template:'<div class="btn-group" role="group" aria-label="First group"><button ng-disabled="!pending.length || position == 0" type="button" class="glyphicon glyphicon-arrow-left btn btn-default btn-recovery" ng-click="previous()"></button><button ng-disabled="!pending.length || position == pending.length" type="button" class="glyphicon glyphicon-arrow-right btn btn-default btn-recovery" ng-click="next()"></button><button ng-disabled="!pending.length || position == 0" type="button" class="glyphicon glyphicon-remove btn btn-default btn-recovery" ng-click="remove()"></button></div>',restrict:"EA",link:function(t){t.position=0,e.getItem(t.resource).then(function(e){t.pending=_(e).filter(function(e){return!e.id}).map(function(e,t){return e.index=t,e}).value()}),t.next=function(){t.position++,t.model=0==t.position?{}:t.pending[t.position-1]},t.previous=function(){t.position--,t.model=0==t.position?{}:t.pending[t.position-1]},t.remove=function(){e.getItem(t.resource).then(function(n){t.pending=_.filter(n,function(e,n){return n!==t.position-1}),e.setItem(t.resource,t.pending),t.previous()})}}}}),angular.module("qualitaCoreFrontend").directive("reportViewer",["$modal","$sce",function(e,t){return{template:"",restrict:"E",scope:{url:"=",title:"@",background:"="},link:function(n,o){n.close=function(){n.modalInstance.dismiss("close")},n.$watch("url",function(){n.url&&(n.trustedUrl=t.trustAsResourceUrl(n.url),n.background?o.append('<iframe src="'+n.trustedUrl+'" hidden></iframe>'):n.modalInstance=e.open({template:'<div class="modal-header"><div class="close glyphicon glyphicon-remove" ng-click="close()"></div><h3 class="modal-title">{{title}}</h3></div><div class="modal-body"><iframe src="{{trustedUrl}}" width="100%" height="450"></iframe></div><div class="modal-footer"><button class="btn btn-primary" ng-click="close()">Cerrar</button></div>',scope:n}))})}}}]),angular.module("qualitaCoreFrontend").config(["schemaFormProvider","schemaFormDecoratorsProvider","sfPathProvider",function(e,t,n){var o=function(t,o,i){if("string"===o.type&&"enum"in o){var r=e.stdFormObj(t,o,i);return r.key=i.path,r.type="strapselect",i.lookup[n.stringify(i.path)]=r,r}};e.defaults.string.unshift(o),t.addMapping("bootstrapDecorator","strapselect","views/directives/strapselect.html"),t.createDirective("strapselect","views/directives/strapselect.html"),t.addMapping("bootstrapDecorator","strapmultiselect","views/directives/strapmultiselect.html"),t.createDirective("strapmultiselect","views/directives/strapmultiselect.html"),t.addMapping("bootstrapDecorator","strapselectdynamic","views/directives/strapselect.html"),t.createDirective("strapselectdynamic","views/directives/strapselect.html"),t.addMapping("bootstrapDecorator","strapmultiselectdynamic","views/directives/strapmultiselect.html"),t.createDirective("strapmultiselectdynamic","views/directives/strapmultiselect.html"),t.addMapping("bootstrapDecorator","uiselect","views/directives/uiselect.html"),t.createDirective("uiselect","views/directives/uiselect.html"),t.addMapping("bootstrapDecorator","uiselectmultiple","views/directives/uiselectmultiple.html"),t.createDirective("uiselectmultiple","views/directives/uiselectmultiple.html")}]).directive("toggleSingleModel",function(){return{require:"ngModel",restrict:"A",scope:{},replace:!0,controller:["$scope",function(e){e.$parent.$watch("select_model.selected",function(){void 0!=e.$parent.select_model.selected&&(e.$parent.insideModel=e.$parent.select_model.selected.value,e.$parent.ngModel.$setViewValue(e.$parent.select_model.selected.value))})}]}}).directive("toggleModel",function(){return{require:"ngModel",restrict:"A",scope:{},controller:["$scope","sfSelect",function(e,t){var n=t(e.$parent.form.key,e.$parent.model);angular.isUndefined(n)&&(n=[],t(e.$parent.form.key,e.$parent.model,n)),e.$parent.$watch("form.selectedOptions",function(){e.$parent.form.selectedOptions&&(0==e.$parent.form.selectedOptions.length?void 0!=e.$parent.ngModel.$viewValue&&e.$parent.ngModel.$setViewValue(e.$parent.form.selectedOptions):(e.$parent.$$value$$=[],e.$parent.form.selectedOptions.forEach(function(t){e.$parent.$$value$$.push(t.value)}),e.$parent.ngModel.$setViewValue(e.$parent.$$value$$)))},!0)}]}}).directive("multipleOn",function(){return{link:function(e,t){e.$watch(function(){return t.attr("multiple-on")},function(e){if("true"==e){var n=angular.element(t).scope().$$childTail;n.$isMultiple=!0,n.options.multiple=!0,n.$select.$element.addClass("select-multiple")}else angular.element(t).scope().$$childTail.$isMultiple=!1})}}}).filter("whereMulti",function(){return function(e,t,n){var o=[];return angular.isArray(n)&&void 0!==e?n.forEach(function(n){for(var i=0;i<e.length;i++)if(n==e[i][t]){o.push(e[i]);break}}):o=e,o}}).filter("propsFilter",function(){return function(e,t){var n=[];return angular.isArray(e)?e.forEach(function(e){for(var o=!1,i=Object.keys(t),r=0;r<i.length;r++){var a=i[r];if(e.hasOwnProperty(a)){var l=t[a].toLowerCase();if(0===e[a].toString().toLowerCase().indexOf(l)||-1!==e[a].toString().toLowerCase().indexOf(" "+l)){o=!0;break}}}o&&n.push(e)}):n=e,n}}).filter("filterRelated",function(){return function(e,t){return"function"==typeof t.form.options.filterRelated?t.form.options.filterRelated(e):e}}),angular.module("qualitaCoreFrontend").controller("dynamicSelectController",["$scope","$http","$timeout",function(e,t,n){function o(t,n,o){var i=r(o),a=o.map;if(angular.isArray(i))return t;if(!i)return[{value:void 0,name:e.form.placeholder}].concat(t);for(var l=0;l<t.length&&t[l][a.valueProperty]!==i[a.valueProperty];l++);var s=t[0];return t[0]=t[l],t[l]=s,t}function r(t){var n=e.form,o=n.key[0],i=angular.element("form[name="+t.formName+"]").scope();return i.model[o]}function a(t,n){var o=r(n);e.select_model.selected={value:o,name:o}}e.select_model={},e.triggerTitleMap=function(){console.log("listener triggered"),e.form.titleMap.push({value:"345890u340598u3405u9",name:"34095u3p4ouij"}),n(function(){e.form.titleMap.pop()})},e.initFiltering=function(t){e.form.options.filterTriggers&&e.form.options.filterTriggers.forEach(function(t){e.$parent.$watch(t,e.triggerTitleMap)}),e.localModelType=Object.prototype.toString.call(t),e.filteringInitialized=!0},e.remap=function(e,t){if(e&&"map"in e&&e.map){var n=[];if(t.forEach(function(t){t.value=t[e.map.valueProperty],t.name=t[e.map.nameProperty],n.push(t)}),e.formName){var i=angular.element("form[name="+e.formName+"]").scope();n=o(t,i,e)}return n}return t.forEach(function(e){"text"in e&&(e.name=e.text)}),t},e.clone=function(t){if(null==t||"object"!=typeof t)return t;var n=t.constructor();for(var o in t)t.hasOwnProperty(o)&&(n[o]=e.clone(t[o]));return n},e.getCallback=function(t){if("string"==typeof t){var n=e.$parent.evalExpr(t);if("function"==typeof n)return n;throw"A callback string must match name of a function in the parent scope"}if("function"==typeof t)return t;throw"A callback must either be a string matching the name of a function in the parent scope or a direct function reference"},e.getOptions=function(t){return t.httpPost&&t.httpPost.optionsCallback?(newOptionInstance=e.clone(t),e.getCallback(t.httpPost.optionsCallback)(newOptionInstance)):t.httpGet&&t.httpGet.optionsCallback?(newOptionInstance=e.clone(t),e.getCallback(t.httpGet.optionsCallback)(newOptionInstance)):t},e.test=function(e){e.titleMap.pop()},e.populateTitleMap=function(n){if("enum"in n.schema)n.titleMap=[],n.schema.enum.forEach(function(e){n.titleMap.push({value:e,name:e})}),a(n.titleMap,n.options);else if(n.options)if(n.options.callback)n.titleMap=e.getCallback(n.options.callback)(n.options),console.log("callback items",n.titleMap);else{if(n.options.asyncCallback)return e.getCallback(n.options.asyncCallback)(n.options).then(function(t){n.titleMap=e.remap(n.options,t.data),n.options.multiple?e.uiMultiSelectInitInternalModel(r(n.options)):e.select_model.selected=n.titleMap[0],console.log("asyncCallback items",n.titleMap)},function(e,t){alert("Loading select items failed(Options: '"+String(n.options)+"\nError: "+t)});if(n.options.httpPost){var o=e.getOptions(n.options);return t.post(o.httpPost.url,o.httpPost.parameter).then(function(t){n.titleMap=e.remap(o,t.data),console.log("httpPost items",n.titleMap)},function(e,t){alert("Loading select items failed (URL: '"+String(o.httpPost.url)+"' Parameter: "+String(o.httpPost.parameter)+"\nError: "+t)})}if(n.options.httpGet){var o=e.getOptions(n.options);return t.get(o.httpGet.url,o.httpGet.parameter).then(function(t){n.titleMap=e.remap(o,t.data),console.log("httpGet items",n.titleMap)},function(e,t){alert("Loading select items failed (URL: '"+String(o.httpGet.url)+"\nError: "+t)})}}else console.log("dynamicSelectController.populateTitleMap(key:"+n.key+") : No options set, needed for dynamic selects")},e.uiMultiSelectInitInternalModel=function(t){function n(t){for(i=0;i<e.form.titleMap.length;i++)if(e.form.titleMap[i].value==t)return e.form.titleMap[i].name}e.internalModel=[],void 0!==t&&angular.isArray(t)&&t.forEach(function(t){e.internalModel.push({value:t,name:n(t)})})}}]),angular.module("qualitaCoreFrontend").filter("selectFilter",[function(){return function(e,t,n,o){if(!angular.isDefined(e)||!angular.isDefined(t.form.options)||!angular.isDefined(t.form.options.filter)||""==t.form.options.filter)return e;console.log("----- In filtering for "+t.form.key+"("+t.form.title+"), model value: "+JSON.stringify(n)+"----"),console.log("Filter:"+t.form.options.filter),t.filteringInitialized||(console.log("Initialize filter"),t.initFiltering(n));var i=[];return angular.forEach(e,function(e){console.log("Compare: curr_item: "+JSON.stringify(e)+"with : "+JSON.stringify(t.$eval(t.form.options.filterTriggers[0]))),t.$eval(t.form.options.filter,{item:e})?i.push(e):n&&("[object Array]"==t.localModelType&&n.indexOf(e.value)>-1?n.splice(n.indexOf(e.value),1):n==e.value&&(console.log("Setting model of type "+t.localModelType+"to null."),n=null))}),"[object Array]"!=t.localModelType||n||(console.log("Resetting model of type "+t.localModelType+" to []."),t.$eval(o+"=[]")),console.log("Input: "+JSON.stringify(e)),console.log("Output: "+JSON.stringify(i)),console.log("Model value out : "+JSON.stringify(n)),console.log("----- Exiting filter for "+t.form.title+"-----"),i}}]),angular.module("qualitaCoreFrontend").directive("tdnDatatable",function(e,t,n,o,i,r,a,l,s,c){var u=r.hasPermission;return{template:'<div><div class="widget"><div class="widget-header"><span class="widget-caption">{{options.title}}</span><div class="widget-buttons"><a href="#" ng-show="canCreate()" ng-click="new()" title="Nuevo"><i class="glyphicon glyphicon-plus"></i></a><a ng-repeat="menuOption in options.extraMenuOptions" href="#" ng-show="menuOption.showCondition()" ng-click="menuOption.action()" title="{{menuOption.title}}"><p><i class="{{menuOption.icon}}"></i>  {{menuOption.data}}&nbsp;&nbsp;&nbsp;</p></a></div></div><div class="widget-body"><div class="table-responsive"><table datatable="" dt-options="dtOptions" dt-columns="dtColumns" dt-instance="dtInstanceCallback" width=100% class="table table-striped no-footer"><tfoot><tr></tr></tfoot></table></div><div ng-if="selected"><h3>Detalles</h3><table class="table table-striped table-bordered table-detail"><tbody><tr ng-repeat="row in options.detailRows"><td ng-if="selected[row.data]" class="row-title">{{row.title}}</td><td ng-if="selected[row.data] && row.renderWith">{{row.renderWith(selected[row.data])}}</td><td ng-if="selected[row.data] && !row.renderWith">{{selected[row.data]}}</td></tr></tbody></table></div></div></div>',restrict:"AE",replace:!0,scope:{options:"="},controller:function(r){function d(t,n){return $("td",t).unbind("click"),$("td",t).bind("click",function(){r.$apply(function(){r.selected=n,e(function(){$(document).scrollTop($(".table-detail").offset().top)})})}),t}var p,f,m=_.template(s.getBaseUrl()+"/<%= resource %>/datatables?");r.dtInstance={},r.selectAll=!1,r.headerCompiled=!1,r.realOrder={},r.customFilters={};var g=function(e,t){var n=i(m(r.options)+$.param(e),{},{query:{isArray:!1}});n.query().$promise.then(function(e){t(e)})},h=r.options.ajax?r.options.ajax:g;r.dtOptions=a.newOptions().withOption("ajax",h).withDataProp("data").withOption("processing",!0).withOption("serverSide",!0).withOption("order",[[r.options.defaultOrderColumn,r.options.defaultOrderDir]]).withOption("language",{sProcessing:"Procesando...",sLengthMenu:"Mostrar _MENU_ registros",sZeroRecords:"No se encontraron resultados",sEmptyTable:"Ningún dato disponible en esta tabla",sInfo:"Mostrando registros del _START_ al _END_ de un total de _TOTAL_ registros",sInfoEmpty:"Mostrando registros del 0 al 0 de un total de 0 registros",sInfoFiltered:"(filtrado de un total de _MAX_ registros)",sInfoPostFix:".",sSearch:"Buscar:",sInfoThousands:",",sLoadingRecords:"Cargando...",oPaginate:{sFirst:"Primero",sLast:"Último",sNext:"Siguiente",sPrevious:"Anterior"},oAria:{sSortAscending:": Activar para ordenar la columna de manera ascendente",sSortDescending:": Activar para ordenar la columna de manera descendente"}}).withOption("createdRow",function(e){n(angular.element(e).contents())(r)}).withOption("headerCallback",function(e){r.headerCompiled||(r.headerCompiled=!0,n(angular.element(e).contents())(r))}).withPaginationType("full_numbers").withButtons(["colvis"]).withColReorder().withColReorderCallback(function(){var e=this.fnOrder();console.log("Columns order has been changed with: "+e),r.realOrder={},_.each(r.dtColumns,function(t,n){var o;_.each(e,function(e,t){e===n&&(o=t)}),r.realOrder[t.sTitle]=o})}).withBootstrap(),r.options.detailRows&&(r.dtOptions=r.dtOptions.withOption("rowCallback",d)),r.visibleColumns=r.options.columns.length,r.dtColumns=[];var v='<label><input type="checkbox" ng-model="selectAll" ng-click="toggleAll(selectAll)"><span class="text"></span></label>';f=l.newColumn(null).withTitle(v).notSortable().withOption("searchable",!1).renderWith(function(e){var t='<label><input id="'+e.id+'" type="checkbox" ng-model="$scope.options.selection['+e.id+']" ng-click="toggleOne($scope.options.selection)"><span class="text"></span></label>';return t}),r.options.isSelectable&&(r.dtColumns.push(f),r.visibleColumns+=1,r.dtOptions.withColReorderOption("iFixedColumnsLeft",1)),r.options.hasOptions&&r.dtOptions.withColReorderOption("iFixedColumnsRight",1),_.map(r.options.columns,function(e){var t=l.newColumn(e.data),n=["data","title","class","renderWith","visible","sortable"];if(e.title&&(t=t.withTitle(e.title)),e.class&&(t=t.withClass(e.class)),e.renderWith&&(t=t.renderWith(e.renderWith)),e.visible===!1&&(t=t.notVisible(),r.visibleColumns-=1),e.sortable===!1&&(t=t.notSortable()),_.forOwn(e,function(e,o){_.contains(n,o)||(t=t.withOption(o,e))}),e.type){var o={filterType:e.type,filterUrl:e.filterUrl};r.customFilters[e.title]=o}r.dtColumns.push(t)}),p=l.newColumn(null).withTitle("Operaciones").notSortable().withOption("searchable",!1).renderWith(function(e){var t='<button class="btn btn-success btn-dt" style="margin-right: 5px;" ng-show="canEdit()" ng-click="edit('+e.id+')">   <span class="glyphicon glyphicon-pencil"></span></button><button class="btn btn-danger btn-dt" style="margin-right: 5px;" ng-show="canRemove()" ng-click="remove('+e.id+')">   <span class="glyphicon glyphicon-trash"></span></button>';return r.options.extraRowOptions&&_.forEach(r.options.extraRowOptions,function(n){var i=_.template(n.templateToRender);r[n.functionName]=n.functionDef,t+=i({dataCustom:e[n.customAttribute],dataId:e.id,$state:o,$scope:r})}),t}),r.canEdit=function(){return u("update_"+r.options.resource)},r.canRemove=function(){return u("delete_"+r.options.resource)},r.canCreate=function(){return u("create_"+r.options.resource)},r.options.hasOptions&&(r.dtColumns.push(p),r.visibleColumns+=1),r.new=function(){var e=_.template("app.<%= resource %>.new");o.go(e(r.options))},r.edit=function(e){var t=_.template("app.<%= resource %>.edit");o.go(t(r.options),{id:e})},r.toggleAll=function(){r.selectAll=r.selectAll?!0:!1,r.selectAll?_.each(b.rows().data(),function(e){r.options.selection[e.id]||$("#"+e.id).click()}):_.each(r.options.selection,function(e,t){e&&$("#"+t).click()})},r.toggleOne=function(e){for(var t in e)if(e.hasOwnProperty(t)&&!e[t])return r.selectAll=!1,void 0;var n=!0;_.each(b.rows().data(),function(e){r.options.selection[e.id]||(n=!1)}),r.selectAll=n,r.options.selection=e};var b,w;r.dtInstanceCallback=function(t){$("thead+tfoot").remove(),w=t.id;for(var n=0;n<r.visibleColumns;n++)$("#"+w+" tfoot tr").append("<th></th>");var o,i;r.options.isSelectable?o=":first":r.options.hasOptions&&(i=":last");var a=function(e,n,o){$("#"+e+" tfoot th").not(n).not(o).each(function(){var t=$("#"+e+" thead th").eq($(this).index()).text(),n=r.customFilters[t];if(n){if("combo"===n.filterType){$(this).html('<div id="'+t+'" name="'+t+'" class="filtro-ancho"></div>');var o=function(e){return e.descripcion},i=function(e){return""===e.descripcion?'<div class="select2-user-result">Todos</div>':'<div class="select2-user-result">'+e.descripcion+"</div>"};$("#"+t).select2({minimumResultsForSearch:-1,id:function(e){return e.codigo},data:function(){return $http({url:s.getBaseUrl()+n.filterUrl,method:"GET"})},ajax:{url:s.getBaseUrl()+"/"+n.filterUrl,dataType:"json",quietMillis:250,params:{headers:{Authorization:c.AuthParams.accessToken}},data:function(e){return{q:e}},results:function(e){return{results:e}},cache:!0},initSelection:function(e,t){$(e).val();$.ajax(s.getBaseUrl()+"/"+n.filterUrl,{dataType:"json",beforeSend:function(e){e.setRequestHeader("Authorization",c.AuthParams.accessToken)}}).done(function(e){t(e)})},formatResult:i,formatSelection:o,escapeMarkup:function(e){return e}}).on("change",function(){var e=$("#"+t).select2("val");e.length>0?b.column(":contains("+t+")").search(e).draw():b.column(":contains("+t+")").search("").draw()})}}else $(this).html('<input id="'+t+'" class="column-filter form-control input-sm" type="text" placeholder="'+t+'" style="min-width:60px; width: 100%;" />')}),$("#"+e+" tfoot").insertAfter("#"+e+" thead"),b=t.DataTable,b.columns().eq(0).each(function(e){$("tfoot input:eq("+e.toString()+")").on("keyup change",function(t){var n,o=this;_.each(r.dtColumns,function(e,t){r.realOrder[o.id]?n=r.realOrder[o.id]:e.sTitle==o.id&&(n=t)});var i=n||e;(this.value.length>=1||13==t.keyCode)&&b.column(i).search(this.value).draw(),""==this.value&&b.column(i).search("").draw()})})};a(w,o,i),_.each(r.dtColumns,function(e,t){if(e.filter){var n=$(".input-sm")[t+1];n.value=e.filter}}),$(".dt-button.buttons-collection.buttons-colvis").text("Columnas"),r.dtOptions.reloadData=function(){$("#"+w).DataTable().ajax.reload()},b.on("draw",function(){e(function(){var e=!0;_.each(b.rows().data(),function(t){r.options.selection[t.id]||(e=!1)}),r.selectAll=e})}),b.on("column-visibility",function(e,n,o,i){console.log("change column visibility %o",i),$("tfoot tr").empty(),w=t.id,i===!1?r.visibleColumns-=1:r.visibleColumns+=1;for(var l=0;l<r.visibleColumns;l++)$("#"+w+" tfoot tr").append("<th></th>");var s,c;r.options.isSelectable?s=":first":r.options.hasOptions&&(c=":last"),a(w,s,c)}),r.dtInstance=t,r.options.getFilters=function(){var e=$("#"+w).dataTable(),t=e.oApi._fnAjaxParameters(e.fnSettings()),n=$.param(t).split("data"),o={};return _.each(n,function(e){if(-1===e.indexOf("draw")){var t=e.substring(e.indexOf("=")+1,e.indexOf("&")),n=e.substring(e.indexOf("=",e.indexOf("value"))+1,e.indexOf("&",e.indexOf("value")));void 0!==t&&void 0!==n&&""!=t&&""!==n&&(o[t]=n)}}),o}},r.remove=function(e){r.selectedItemId=e,r.tituloModal="Confirmación de Borrado",r.mensajeModal="Esta operación eliminará el registro seleccionado. ¿Desea continuar?";var n=t.open({template:'<div class="modal-header"><h3 class="modal-title">{{::tituloModal}}</h3></div><div class="modal-body">{{::mensajeModal}}</div><div class="modal-footer"><button class="btn btn-primary" ng-click="ok(selectedItemId)">Aceptar</button><button class="btn btn-warning" ng-click="cancel()">Cancelar</button></div>',scope:r});r.cancel=function(){n.dismiss("cancel")},r.ok=function(e){var t=r.options.factory.create({id:e});r.options.factory.remove(t).then(function(){r.dtOptions.reloadData(),n.close(e)})}},r.options.detailRows&&(r.options.detailRows=r.options.detailRows===!0?r.options.columns:_.union(r.options.columns,r.options.detailRows))}}}),angular.module("qualitaCoreFrontend").service("AuthenticationService",function(e,t,n,o){var i=e(o.getBaseUrl()+"/:action",{action:"@action"});return{login:function(e,n){t.auxiliarUsername=e;var o=new i({username:e,password:n});return o.$save({action:"login"})},postLogin:function(e){return new i.save({action:"loginApp"},{username:e.username})},token:function(e){var t=new i({username:e.username,accessToken:e.accessToken,requestToken:e.requestToken});return t.$save({action:"token"})},logout:function(){var e=this.getCurrentUser(),n=new i({username:e.username,accessToken:e.accessToken});return t.AuthParams={},localStorage.removeItem("AUTH_PARAMS"),n.$save({action:"logout"})},getCurrentUser:function(){var e=t.AuthParams;return e&&0!==Object.keys(e).length||(e=JSON.parse(localStorage.getItem("AUTH_PARAMS"))||void 0,e&&(n.defaults.headers.common.Authorization="Bearer "+e.accessToken)),e}}}),angular.module("qualitaCoreFrontend").service("AuthorizationService",function(e,t,n,o,i){var r=t(o.getBaseUrl()+"/authorization/:action",{action:"@action"});return{hasPermission:function(e,t){var n=t||i.getCurrentUser(),o=[];return n&&(o=n.permissions||[]),o.indexOf(e)>=0},principal:function(){return r.get({action:"principal"}).$promise},setupCredentials:function(t,o,i,r){var a={username:t,requestToken:o,accessToken:i};e.AuthParams=a,localStorage.setItem("AUTH_PARAMS",JSON.stringify(a)),n.defaults.headers.common.Authorization="Bearer "+i,this.principal().then(function(e){a.permissions=e.permisos,a.stamp=e.stamp,localStorage.setItem("AUTH_PARAMS",JSON.stringify(a)),r(a)})},cleanupCredentials:function(){localStorage.removeItem("AUTH_PARAMS")},authorize:function(e,t){var n=i.getCurrentUser();if(e===!0&&void 0===n)return this.enums.LOGIN_REQUIRED;if(e&&void 0!==n&&(void 0===t||0===t.length))return this.enums.AUTHORIZED;if(t){for(var o=!0,r=0;r<t.length&&(o=this.hasPermission(t[r],n),o!==!1);r++);return o?this.enums.AUTHORIZED:this.enums.NOT_AUTHORIZED}},enums:{LOGIN_REQUIRED:"loginRequired",NOT_AUTHORIZED:"notAuthorized",AUTHORIZED:"authorized"}}}),angular.module("qualitaCoreFrontend").factory("baseurl",function(e){return{getBaseUrl:function(){var t=window.location.hostname;return t===e.serverIp?"http://"+t+"/"+e.serverName+"/rest":"http://"+t+":8088/"+e.serverName+"/rest"},getPublicBaseUrl:function(){var t=window.location.hostname;return t===e.serverIp?"http://"+t+"/public/":"http://"+t+":8088/public/"}}}),angular.module("qualitaCoreFrontend").config(["schemaFormProvider","schemaFormDecoratorsProvider","sfPathProvider","flowFactoryProvider",function(e,t,n,o){o.defaults={method:"octet"};var i=function(t,o,i){if("object"===o.type&&"fileupload"===o.format){var r=e.stdFormObj(t,o,i);return r.key=i.path,r.type="fileupload",i.lookup[n.stringify(i.path)]=r,r}};e.defaults.object.unshift(i),t.addMapping("bootstrapDecorator","fileupload","views/directives/fileupload.html"),t.createDirective("fileupload","views/directives/fileupload.html")}]).factory("fileupload",function(){}),angular.module("qualitaCoreFrontend").factory("filterFactory",function(){var e=function(e,t){var i={_inner:{type:e}};return i._inner.filters=this&&this._inner?[this._inner,t]:t,i.or||"and"!==e||(i.or=n),i.value||(i.value=r),i.add||(i.add=o),i.paginate=a,i},t=function(t){return e.call(this,"and",t)},n=function(t){return e.call(this,"or",t)},o=function(e){return this.filters.push(e),this},i=function(e){return t([e])},r=function(){return this._inner},a=function(e,t){return this._inner.limit=e,this._inner.offset=t,this};return{and:t,or:n,add:o,single:i,value:r}}),angular.module("qualitaCoreFrontend").factory("formFactory",function(e,t,n){return{defaultForm:function(){return["*",{type:"submit",title:"Guardar",htmlClass:"pull-right"}]},defaultOptions:function(){return{formDefaults:{ngModelOptions:{updateOn:"blur"}},validationMessage:{302:"El campo es obligatorio"}}},defaultSubmit:function(o,i,r,a,l){if(i.$broadcast("schemaFormValidate"),r.$valid){if(i.model)var s=a.create(i.model);else var s=a.create(l.model);a.save(s).then(function(){e.path("/"+o)},function(){var e="Error al persistir la operación.";i.model.id||(e+="\n\nGuardando localmente, reintente más tarde."),n({message:e,classes:"alert-danger",position:"right"}),t.getItem(o).then(function(e){e=e||[],e.unshift(i.model),i.model.id||t.setItem(o,e)})})}}}}),angular.module("qualitaCoreFrontend").factory("HttpInterceptor",function(e,t,n,o,i){return{request:function(e){return"/login"!==t.path()&&(e.headers.Authorization="Bearer "+n.AuthParams.accessToken),e},requestError:function(n){return 401===n.status&&t.path("/login"),e.reject(n)},response:function(e){return e},responseError:function(o){var r=i.get("notify");if(401===o.status){if(o.data&&403===o.data.code)return r({message:o.data.error,classes:["alert-danger"]}),t.path("/"),e.reject(o);if("/login"===t.path())return e.reject(o);var a=e.defer(),l=i.get("AuthenticationService"),s=i.get("$http"),c=l.token(n.AuthParams);return c.then(function(e){n.AuthParams.accessToken=e.accessToken,localStorage.setItem("AUTH_PARAMS",JSON.stringify(n.AuthParams)),s.defaults.headers.common.Authorization="Bearer "+e.accessToken,l.postLogin(n.AuthParams).$promise.then(function(e){n.AuthParams.accesoSistema=e,s.defaults.headers.common["X-Access"]=n.AuthParams.accesoSistema.accesosSistema[0].locacion.id})}).then(a.resolve,a.reject),a.promise.then(function(){return o.config.headers.Authorization="Bearer "+n.AuthParams.accessToken,s(o.config)})}return e.reject(o)}}}),angular.module("qualitaCoreFrontend").factory("ReportTicketFactory",["$resource","baseurl",function(e,t){var n=e(t.getBaseUrl()+"/ticket/:reportID",{action:"@reportID"});return{ticket:function(e,t){var o=new n(t);return o.$save({reportID:e})},downloadURL:function(e,n){return console.log("downloadURL"),t.getBaseUrl()+"/generar/"+e+"/"+n}}}]),angular.module("qualitaCoreFrontend").factory("usuariosFactory",function(e,t,n){var o=e(n.getBaseUrl()+"/usuarios/:id",{id:"@id"},{update:{method:"PUT"}});return{all:function(e){return o.query(e)},get:function(e){return o.get({id:e})},getByUsername:function(e){var n={};return n.search=t.single({path:"username",equals:e}).value(),o.query(n)},create:function(e){return new o(e)},save:function(e){return e.id?e.$update():e.$save()},remove:function(e){return e.$remove()}}});
=======
!function(e){e.module("qualitaCoreFrontend.config",[]).value("qualitaCoreFrontend.config",{debug:!0}),e.module("qualitaCoreFrontend.directives",[]),e.module("qualitaCoreFrontend.filters",[]),e.module("qualitaCoreFrontend.services",[]),e.module("qualitaCoreFrontend",["qualitaCoreFrontend.config","qualitaCoreFrontend.directives","qualitaCoreFrontend.filters","qualitaCoreFrontend.services","ngResource","ngCookies","ngSanitize","ngTouch","datatables","datatables.bootstrap","schemaForm","pascalprecht.translate","cgNotify","angular-underscore/filters","flow","ui.bootstrap","ui.select","ui.highlight","mgcrea.ngStrap.select","ncy-angular-breadcrumb","ui.router","oc.lazyLoad","ngStorage","LocalForageModule","datatables.buttons","datatables.colreorder","daterangepicker"])}(angular),angular.module("qualitaCoreFrontend").directive("fileupload",["$rootScope",function(e){return{template:'<div ng-show="form.uploadOptions.imageOnly"><div flow-init="{singleFile: true, target: form.uploadOptions.target}" flow-file-added="!!{png:1,gif:1,jpg:1,jpeg:1}[$file.getExtension()]"flow-files-submitted="form.uploader.flow.upload()"flow-files-added="filesAdded($files, $event, form.uploader.flow)"flow-name="form.uploader.flow"class="ng-scope"><h3>{{showTitle()}}</h3><div class="thumbnail" ng-show="form.uploader.currentFile && !form.uploader.flow.files.length"><img src="{{form.uploader.currentFile}}"/> </div><div class="thumbnail" ng-show="!form.uploader.flow.files.length && !form.uploader.currentFile"><img src="images/placeholder.png"></div><div class="thumbnail" ng-show="form.uploader.flow.files.length"><img flow-img="form.uploader.flow.files[0]"/> </div><div><span class="btn btn-primary" ng-show="!form.uploader.flow.files.length" flow-btn="">Seleccionar imagen<input type="file" sf-changed="form" ng-model="$$value$$" style="visibility: hidden; position: absolute;" /></span><span class="btn btn-info ng-hide" ng-show="form.uploader.flow.files.length" flow-btn="">Cambiar<input type="file" sf-changed="form" ng-model="$$value$$" style="visibility: hidden; position: absolute;" /></span><span class="btn btn-danger ng-hide" ng-show="form.uploader.flow.files.length" ng-click="form.uploader.flow.cancel()">Eliminar</span></div><p>Formatos permitidos: PNG, GIF, JPG y JPEG.</p></div></div><div ng-show="!form.uploadOptions.imageOnly"><div flow-init="{singleFile: true, target: form.uploadOptions.target}" flow-file-added="filesAdded($files, $event, uploader.flow)"flow-files-submitted="form.uploader.flow.upload()"flow-files-added="filesAdded($files, $event, form.uploader.flow)"flow-name="form.uploader.flow"class="ng-scope"><h3>{{showTitle()}}</h3><div class="drop" flow-drop ng-class="dropClass"><span class="btn btn-default" flow-btn>Cargar archivo<input type="file" ng-model="$$value$$" sf-changed="form" style="visibility: hidden; position: absolute;" /></span><b>OR</b>Arrastre el archivo aqu&iacute;</div><br/><div><div ng-repeat="file in form.uploader.flow.files" class="transfer-box">{{file.relativePath}} ({{file.size}}bytes)<div class="progress progress-striped" ng-class="{active: file.isUploading()}"><div class="progress-bar" role="progressbar"aria-valuenow="{{file.progress() * 100}}"aria-valuemin="0"aria-valuemax="100"NaN<span class="sr-only">{{file.progress()}}% Complete</span></div></div><div class="btn-group"><a class="btn btn-xs btn-danger" ng-click="file.cancel()">Cancelar</a><a class="btn btn-xs btn-info" ng-click="file.retry()" ng-show="file.error">Reintentar</a></div></div></div></div></div>',restrict:"E",tranclude:!0,scope:!0,link:function(t,n,o){t.uploader={},t.title=o.title,t.fileModel={},t.filesAdded=function(t,n,o){e.flow||(e.flow=o)}}}}]),angular.module("qualitaCoreFrontend").directive("offlineFormRecovery",function(e){return{template:'<div class="btn-group" role="group" aria-label="First group"><button ng-disabled="!pending.length || position == 0" type="button" class="glyphicon glyphicon-arrow-left btn btn-default btn-recovery" ng-click="previous()"></button><button ng-disabled="!pending.length || position == pending.length" type="button" class="glyphicon glyphicon-arrow-right btn btn-default btn-recovery" ng-click="next()"></button><button ng-disabled="!pending.length || position == 0" type="button" class="glyphicon glyphicon-remove btn btn-default btn-recovery" ng-click="remove()"></button></div>',restrict:"EA",link:function(t){t.position=0,e.getItem(t.resource).then(function(e){t.pending=_(e).filter(function(e){return!e.id}).map(function(e,t){return e.index=t,e}).value()}),t.next=function(){t.position++,t.model=0==t.position?{}:t.pending[t.position-1]},t.previous=function(){t.position--,t.model=0==t.position?{}:t.pending[t.position-1]},t.remove=function(){e.getItem(t.resource).then(function(n){t.pending=_.filter(n,function(e,n){return n!==t.position-1}),e.setItem(t.resource,t.pending),t.previous()})}}}}),angular.module("qualitaCoreFrontend").directive("reportViewer",["$modal","$sce",function(e,t){return{template:"",restrict:"E",scope:{url:"=",title:"@",background:"="},link:function(n,o){n.close=function(){n.modalInstance.dismiss("close")},n.$watch("url",function(){n.url&&(n.trustedUrl=t.trustAsResourceUrl(n.url),n.background?o.append('<iframe src="'+n.trustedUrl+'" hidden></iframe>'):n.modalInstance=e.open({template:'<div class="modal-header"><div class="close glyphicon glyphicon-remove" ng-click="close()"></div><h3 class="modal-title">{{title}}</h3></div><div class="modal-body"><iframe src="{{trustedUrl}}" width="100%" height="450"></iframe></div><div class="modal-footer"><button class="btn btn-primary" ng-click="close()">Cerrar</button></div>',scope:n}))})}}}]),angular.module("qualitaCoreFrontend").config(["schemaFormProvider","schemaFormDecoratorsProvider","sfPathProvider",function(e,t,n){var o=function(t,o,i){if("string"===o.type&&"enum"in o){var r=e.stdFormObj(t,o,i);return r.key=i.path,r.type="strapselect",i.lookup[n.stringify(i.path)]=r,r}};e.defaults.string.unshift(o),t.addMapping("bootstrapDecorator","strapselect","views/directives/strapselect.html"),t.createDirective("strapselect","views/directives/strapselect.html"),t.addMapping("bootstrapDecorator","strapmultiselect","views/directives/strapmultiselect.html"),t.createDirective("strapmultiselect","views/directives/strapmultiselect.html"),t.addMapping("bootstrapDecorator","strapselectdynamic","views/directives/strapselect.html"),t.createDirective("strapselectdynamic","views/directives/strapselect.html"),t.addMapping("bootstrapDecorator","strapmultiselectdynamic","views/directives/strapmultiselect.html"),t.createDirective("strapmultiselectdynamic","views/directives/strapmultiselect.html"),t.addMapping("bootstrapDecorator","uiselect","views/directives/uiselect.html"),t.createDirective("uiselect","views/directives/uiselect.html"),t.addMapping("bootstrapDecorator","uiselectmultiple","views/directives/uiselectmultiple.html"),t.createDirective("uiselectmultiple","views/directives/uiselectmultiple.html")}]).directive("toggleSingleModel",function(){return{require:"ngModel",restrict:"A",scope:{},replace:!0,controller:["$scope",function(e){e.$parent.$watch("select_model.selected",function(){void 0!=e.$parent.select_model.selected&&(e.$parent.insideModel=e.$parent.select_model.selected.value,e.$parent.ngModel.$setViewValue(e.$parent.select_model.selected.value))})}]}}).directive("toggleModel",function(){return{require:"ngModel",restrict:"A",scope:{},controller:["$scope","sfSelect",function(e,t){var n=t(e.$parent.form.key,e.$parent.model);angular.isUndefined(n)&&(n=[],t(e.$parent.form.key,e.$parent.model,n)),e.$parent.$watch("form.selectedOptions",function(){e.$parent.form.selectedOptions&&(0==e.$parent.form.selectedOptions.length?void 0!=e.$parent.ngModel.$viewValue&&e.$parent.ngModel.$setViewValue(e.$parent.form.selectedOptions):(e.$parent.$$value$$=[],e.$parent.form.selectedOptions.forEach(function(t){e.$parent.$$value$$.push(t.value)}),e.$parent.ngModel.$setViewValue(e.$parent.$$value$$)))},!0)}]}}).directive("multipleOn",function(){return{link:function(e,t){e.$watch(function(){return t.attr("multiple-on")},function(e){if("true"==e){var n=angular.element(t).scope().$$childTail;n.$isMultiple=!0,n.options.multiple=!0,n.$select.$element.addClass("select-multiple")}else angular.element(t).scope().$$childTail.$isMultiple=!1})}}}).filter("whereMulti",function(){return function(e,t,n){var o=[];return angular.isArray(n)&&void 0!==e?n.forEach(function(n){for(var i=0;i<e.length;i++)if(n==e[i][t]){o.push(e[i]);break}}):o=e,o}}).filter("propsFilter",function(){return function(e,t){var n=[];return angular.isArray(e)?e.forEach(function(e){for(var o=!1,i=Object.keys(t),r=0;r<i.length;r++){var a=i[r];if(e.hasOwnProperty(a)){var l=t[a].toLowerCase();if(0===e[a].toString().toLowerCase().indexOf(l)||-1!==e[a].toString().toLowerCase().indexOf(" "+l)){o=!0;break}}}o&&n.push(e)}):n=e,n}}).filter("filterRelated",function(){return function(e,t){return"function"==typeof t.form.options.filterRelated?t.form.options.filterRelated(e):e}}),angular.module("qualitaCoreFrontend").controller("dynamicSelectController",["$scope","$http","$timeout",function(e,t,n){function o(t,n,o){var i=r(o),a=o.map;if(angular.isArray(i))return t;if(!i)return[{value:void 0,name:e.form.placeholder}].concat(t);for(var l=0;l<t.length&&t[l][a.valueProperty]!==i[a.valueProperty];l++);var s=t[0];return t[0]=t[l],t[l]=s,t}function r(t){var n=e.form,o=n.key[0],i=angular.element("form[name="+t.formName+"]").scope();return i.model[o]}function a(t,n){var o=r(n);e.select_model.selected={value:o,name:o}}e.select_model={},e.triggerTitleMap=function(){console.log("listener triggered"),e.form.titleMap.push({value:"345890u340598u3405u9",name:"34095u3p4ouij"}),n(function(){e.form.titleMap.pop()})},e.initFiltering=function(t){e.form.options.filterTriggers&&e.form.options.filterTriggers.forEach(function(t){e.$parent.$watch(t,e.triggerTitleMap)}),e.localModelType=Object.prototype.toString.call(t),e.filteringInitialized=!0},e.remap=function(e,t){if(e&&"map"in e&&e.map){var n=[];if(t.forEach(function(t){t.value=t[e.map.valueProperty],t.name=t[e.map.nameProperty],n.push(t)}),e.formName){var i=angular.element("form[name="+e.formName+"]").scope();n=o(t,i,e)}return n}return t.forEach(function(e){"text"in e&&(e.name=e.text)}),t},e.clone=function(t){if(null==t||"object"!=typeof t)return t;var n=t.constructor();for(var o in t)t.hasOwnProperty(o)&&(n[o]=e.clone(t[o]));return n},e.getCallback=function(t){if("string"==typeof t){var n=e.$parent.evalExpr(t);if("function"==typeof n)return n;throw"A callback string must match name of a function in the parent scope"}if("function"==typeof t)return t;throw"A callback must either be a string matching the name of a function in the parent scope or a direct function reference"},e.getOptions=function(t){return t.httpPost&&t.httpPost.optionsCallback?(newOptionInstance=e.clone(t),e.getCallback(t.httpPost.optionsCallback)(newOptionInstance)):t.httpGet&&t.httpGet.optionsCallback?(newOptionInstance=e.clone(t),e.getCallback(t.httpGet.optionsCallback)(newOptionInstance)):t},e.test=function(e){e.titleMap.pop()},e.populateTitleMap=function(n){if("enum"in n.schema)n.titleMap=[],n.schema.enum.forEach(function(e){n.titleMap.push({value:e,name:e})}),a(n.titleMap,n.options);else if(n.options)if(n.options.callback)n.titleMap=e.getCallback(n.options.callback)(n.options),console.log("callback items",n.titleMap);else{if(n.options.asyncCallback)return e.getCallback(n.options.asyncCallback)(n.options).then(function(t){n.titleMap=e.remap(n.options,t.data),n.options.multiple?e.uiMultiSelectInitInternalModel(r(n.options)):e.select_model.selected=n.titleMap[0],console.log("asyncCallback items",n.titleMap)},function(e,t){alert("Loading select items failed(Options: '"+String(n.options)+"\nError: "+t)});if(n.options.httpPost){var o=e.getOptions(n.options);return t.post(o.httpPost.url,o.httpPost.parameter).then(function(t){n.titleMap=e.remap(o,t.data),console.log("httpPost items",n.titleMap)},function(e,t){alert("Loading select items failed (URL: '"+String(o.httpPost.url)+"' Parameter: "+String(o.httpPost.parameter)+"\nError: "+t)})}if(n.options.httpGet){var o=e.getOptions(n.options);return t.get(o.httpGet.url,o.httpGet.parameter).then(function(t){n.titleMap=e.remap(o,t.data),console.log("httpGet items",n.titleMap)},function(e,t){alert("Loading select items failed (URL: '"+String(o.httpGet.url)+"\nError: "+t)})}}else console.log("dynamicSelectController.populateTitleMap(key:"+n.key+") : No options set, needed for dynamic selects")},e.uiMultiSelectInitInternalModel=function(t){function n(t){for(i=0;i<e.form.titleMap.length;i++)if(e.form.titleMap[i].value==t)return e.form.titleMap[i].name}e.internalModel=[],void 0!==t&&angular.isArray(t)&&t.forEach(function(t){e.internalModel.push({value:t,name:n(t)})})}}]),angular.module("qualitaCoreFrontend").filter("selectFilter",[function(){return function(e,t,n,o){if(!angular.isDefined(e)||!angular.isDefined(t.form.options)||!angular.isDefined(t.form.options.filter)||""==t.form.options.filter)return e;console.log("----- In filtering for "+t.form.key+"("+t.form.title+"), model value: "+JSON.stringify(n)+"----"),console.log("Filter:"+t.form.options.filter),t.filteringInitialized||(console.log("Initialize filter"),t.initFiltering(n));var i=[];return angular.forEach(e,function(e){console.log("Compare: curr_item: "+JSON.stringify(e)+"with : "+JSON.stringify(t.$eval(t.form.options.filterTriggers[0]))),t.$eval(t.form.options.filter,{item:e})?i.push(e):n&&("[object Array]"==t.localModelType&&n.indexOf(e.value)>-1?n.splice(n.indexOf(e.value),1):n==e.value&&(console.log("Setting model of type "+t.localModelType+"to null."),n=null))}),"[object Array]"!=t.localModelType||n||(console.log("Resetting model of type "+t.localModelType+" to []."),t.$eval(o+"=[]")),console.log("Input: "+JSON.stringify(e)),console.log("Output: "+JSON.stringify(i)),console.log("Model value out : "+JSON.stringify(n)),console.log("----- Exiting filter for "+t.form.title+"-----"),i}}]),angular.module("qualitaCoreFrontend").directive("tdnDatatable",function(e,t,n,o,i,r,a,l,s,c){var u=r.hasPermission;return{template:'<div><div class="widget"><div class="widget-header"><span class="widget-caption">{{options.title}}</span><div class="widget-buttons"><a href="#" ng-show="canCreate()" ng-click="new()" title="Nuevo"><i class="glyphicon glyphicon-plus"></i></a><a ng-repeat="menuOption in options.extraMenuOptions" href="#" ng-show="menuOption.showCondition()" ng-click="menuOption.action()" title="{{menuOption.title}}"><p><i class="{{menuOption.icon}}"></i>  {{menuOption.data}}&nbsp;&nbsp;&nbsp;</p></a></div></div><div class="widget-body"><div class="table-responsive"><table datatable="" dt-options="dtOptions" dt-columns="dtColumns" dt-instance="dtInstanceCallback" width=100% class="table table-striped no-footer"><tfoot><tr></tr></tfoot></table></div><div ng-if="selected"><h3>Detalles</h3><table class="table table-striped table-bordered table-detail"><tbody><tr ng-repeat="row in options.detailRows"><td ng-if="selected[row.data]" class="row-title">{{row.title}}</td><td ng-if="selected[row.data] && row.renderWith">{{row.renderWith(selected[row.data])}}</td><td ng-if="selected[row.data] && !row.renderWith">{{selected[row.data]}}</td></tr></tbody></table></div></div></div>',restrict:"AE",replace:!0,scope:{options:"="},controller:function(r){function d(t,n){return $("td",t).unbind("click"),$("td",t).bind("click",function(){r.$apply(function(){r.selected=n,e(function(){$(document).scrollTop($(".table-detail").offset().top)})})}),t}var p,f,m=_.template(s.getBaseUrl()+"/<%= resource %>/datatables?");r.dtInstance={},r.selectAll=!1,r.headerCompiled=!1,r.realOrder={},r.customFilters={};var g=function(e,t){var n=i(m(r.options)+$.param(e),{},{query:{isArray:!1}});n.query().$promise.then(function(e){t(e)})},h=r.options.ajax?r.options.ajax:g;r.dtOptions=a.newOptions().withOption("ajax",h).withDataProp("data").withOption("processing",!0).withOption("serverSide",!0).withOption("order",[[r.options.defaultOrderColumn,r.options.defaultOrderDir]]).withOption("language",{sProcessing:"Procesando...",sLengthMenu:"Mostrar _MENU_ registros",sZeroRecords:"No se encontraron resultados",sEmptyTable:"Ningún dato disponible en esta tabla",sInfo:"Mostrando registros del _START_ al _END_ de un total de _TOTAL_ registros",sInfoEmpty:"Mostrando registros del 0 al 0 de un total de 0 registros",sInfoFiltered:"(filtrado de un total de _MAX_ registros)",sInfoPostFix:".",sSearch:"Buscar:",sInfoThousands:",",sLoadingRecords:"Cargando...",oPaginate:{sFirst:"Primero",sLast:"Último",sNext:"Siguiente",sPrevious:"Anterior"},oAria:{sSortAscending:": Activar para ordenar la columna de manera ascendente",sSortDescending:": Activar para ordenar la columna de manera descendente"}}).withOption("createdRow",function(e){n(angular.element(e).contents())(r)}).withOption("headerCallback",function(e){r.headerCompiled||(r.headerCompiled=!0,n(angular.element(e).contents())(r))}).withPaginationType("full_numbers").withButtons(["colvis"]).withColReorder().withColReorderCallback(function(){var e=this.fnOrder();console.log("Columns order has been changed with: "+e),r.realOrder={},_.each(r.dtColumns,function(t,n){var o;_.each(e,function(e,t){e===n&&(o=t)}),r.realOrder[t.sTitle]=o})}).withBootstrap(),r.options.detailRows&&(r.dtOptions=r.dtOptions.withOption("rowCallback",d)),r.visibleColumns=r.options.columns.length,r.dtColumns=[];var v='<label><input type="checkbox" ng-model="selectAll" ng-click="toggleAll(selectAll)"><span class="text"></span></label>';f=l.newColumn(null).withTitle(v).notSortable().withOption("searchable",!1).renderWith(function(e){var t='<label><input id="'+e.id+'" type="checkbox" ng-model="$scope.options.selection['+e.id+']" ng-click="toggleOne($scope.options.selection)"><span class="text"></span></label>';return t}),r.options.isSelectable&&(r.dtColumns.push(f),r.visibleColumns+=1,r.dtOptions.withColReorderOption("iFixedColumnsLeft",1)),r.options.hasOptions&&r.dtOptions.withColReorderOption("iFixedColumnsRight",1),_.map(r.options.columns,function(e){var t=l.newColumn(e.data),n=["data","title","class","renderWith","visible","sortable"];if(e.title&&(t=t.withTitle(e.title)),e.class&&(t=t.withClass(e.class)),e.renderWith&&(t=t.renderWith(e.renderWith)),e.visible===!1&&(t=t.notVisible(),r.visibleColumns-=1),e.sortable===!1&&(t=t.notSortable()),_.forOwn(e,function(e,o){_.contains(n,o)||(t=t.withOption(o,e))}),e.type){var o={filterType:e.type,filterUrl:e.filterUrl};r.customFilters[e.title]=o}r.dtColumns.push(t)}),p=l.newColumn(null).withTitle("Operaciones").notSortable().withOption("searchable",!1).renderWith(function(e){var t='<button class="btn btn-success btn-dt" style="margin-right: 5px;" ng-show="canEdit()" ng-click="edit('+e.id+')">   <span class="glyphicon glyphicon-pencil"></span></button><button class="btn btn-danger btn-dt" style="margin-right: 5px;" ng-show="canRemove()" ng-click="remove('+e.id+')">   <span class="glyphicon glyphicon-trash"></span></button>';return r.options.extraRowOptions&&_.forEach(r.options.extraRowOptions,function(n){var i=_.template(n.templateToRender);r[n.functionName]=n.functionDef,t+=i({dataCustom:e[n.customAttribute],dataId:e.id,$state:o,$scope:r})}),t}),r.canEdit=function(){return u("update_"+r.options.resource)},r.canRemove=function(){return u("delete_"+r.options.resource)},r.canCreate=function(){return u("create_"+r.options.resource)},r.options.hasOptions&&(r.dtColumns.push(p),r.visibleColumns+=1),r.new=function(){var e=_.template("app.<%= resource %>.new");o.go(e(r.options))},r.edit=function(e){var t=_.template("app.<%= resource %>.edit");o.go(t(r.options),{id:e})},r.toggleAll=function(){r.selectAll=r.selectAll?!0:!1,r.selectAll?_.each(b.rows().data(),function(e){r.options.selection[e.id]||$("#"+e.id).click()}):_.each(r.options.selection,function(e,t){e&&$("#"+t).click()})},r.toggleOne=function(e){for(var t in e)if(e.hasOwnProperty(t)&&!e[t])return r.selectAll=!1,void 0;var n=!0;_.each(b.rows().data(),function(e){r.options.selection[e.id]||(n=!1)}),r.selectAll=n,r.options.selection=e};var b,w;r.dtInstanceCallback=function(t){$("thead+tfoot").remove(),w=t.id;for(var n=0;n<r.visibleColumns;n++)$("#"+w+" tfoot tr").append("<th></th>");var o,i;r.options.isSelectable?o=":first":r.options.hasOptions&&(i=":last");var a=function(e,n,o){$("#"+e+" tfoot th").not(n).not(o).each(function(){var t=$("#"+e+" thead th").eq($(this).index()).text(),n=r.customFilters[t];if(n){if("combo"===n.filterType){$(this).html('<div id="'+t+'" name="'+t+'" class="filtro-ancho"></div>');var o=function(e){return e.descripcion},i=function(e){return""===e.descripcion?'<div class="select2-user-result">Todos</div>':'<div class="select2-user-result">'+e.descripcion+"</div>"};$("#"+t).select2({minimumResultsForSearch:-1,id:function(e){return e.codigo},data:function(){return $http({url:s.getBaseUrl()+n.filterUrl,method:"GET"})},ajax:{url:s.getBaseUrl()+"/"+n.filterUrl,dataType:"json",quietMillis:250,params:{headers:{Authorization:c.AuthParams.accessToken}},data:function(e){return{q:e}},results:function(e){return{results:e}},cache:!0},initSelection:function(e,t){$(e).val();$.ajax(s.getBaseUrl()+"/"+n.filterUrl,{dataType:"json",beforeSend:function(e){e.setRequestHeader("Authorization",c.AuthParams.accessToken)}}).done(function(e){t(e)})},formatResult:i,formatSelection:o,escapeMarkup:function(e){return e}}).on("change",function(){var e=$("#"+t).select2("val");e.length>0?b.column(":contains("+t+")").search(e).draw():b.column(":contains("+t+")").search("").draw()})}}else $(this).html('<input id="'+t+'" class="column-filter form-control input-sm" type="text" placeholder="'+t+'" style="min-width:60px; width: 100%;" />')}),$("#"+e+" tfoot").insertAfter("#"+e+" thead"),b=t.DataTable,b.columns().eq(0).each(function(e){$("tfoot input:eq("+e.toString()+")").on("keyup change",function(t){var n,o=this;_.each(r.dtColumns,function(e,t){r.realOrder[o.id]?n=r.realOrder[o.id]:e.sTitle==o.id&&(n=t)});var i=n||e;(this.value.length>=1||13==t.keyCode)&&b.column(i).search(this.value).draw(),""==this.value&&b.column(i).search("").draw()})})};a(w,o,i),_.each(r.dtColumns,function(e,t){if(e.filter){var n=$(".input-sm")[t+1];n.value=e.filter}}),$(".dt-buttons").append("<label class='view-columns'>Vistas&nbsp;</label>"),$(".dt-button").addClass("form-control input-sm").text("Columnas"),r.dtOptions.reloadData=function(){$("#"+w).DataTable().ajax.reload()},b.on("draw",function(){e(function(){var e=!0;_.each(b.rows().data(),function(t){r.options.selection[t.id]||(e=!1)}),r.selectAll=e})}),b.on("column-visibility",function(e,n,o,i){console.log("change column visibility %o",i),$("tfoot tr").empty(),w=t.id,i===!1?r.visibleColumns-=1:r.visibleColumns+=1;for(var l=0;l<r.visibleColumns;l++)$("#"+w+" tfoot tr").append("<th></th>");var s,c;r.options.isSelectable?s=":first":r.options.hasOptions&&(c=":last"),a(w,s,c)}),r.dtInstance=t,r.options.getFilters=function(){var e=$("#"+w).dataTable(),t=e.oApi._fnAjaxParameters(e.fnSettings()),n=$.param(t).split("data"),o={};return _.each(n,function(e){if(-1===e.indexOf("draw")){var t=e.substring(e.indexOf("=")+1,e.indexOf("&")),n=e.substring(e.indexOf("=",e.indexOf("value"))+1,e.indexOf("&",e.indexOf("value")));void 0!==t&&void 0!==n&&""!=t&&""!==n&&(o[t]=n)}}),o}},r.remove=function(e){r.selectedItemId=e,r.tituloModal="Confirmación de Borrado",r.mensajeModal="Esta operación eliminará el registro seleccionado. ¿Desea continuar?";var n=t.open({template:'<div class="modal-header"><h3 class="modal-title">{{::tituloModal}}</h3></div><div class="modal-body">{{::mensajeModal}}</div><div class="modal-footer"><button class="btn btn-primary" ng-click="ok(selectedItemId)">Aceptar</button><button class="btn btn-warning" ng-click="cancel()">Cancelar</button></div>',scope:r});r.cancel=function(){n.dismiss("cancel")},r.ok=function(e){var t=r.options.factory.create({id:e});r.options.factory.remove(t).then(function(){r.dtOptions.reloadData(),n.close(e)})}},r.options.detailRows&&(r.options.detailRows=r.options.detailRows===!0?r.options.columns:_.union(r.options.columns,r.options.detailRows))}}}),angular.module("qualitaCoreFrontend").service("AuthenticationService",function(e,t,n,o){var i=e(o.getBaseUrl()+"/:action",{action:"@action"});return{login:function(e,n){t.auxiliarUsername=e;var o=new i({username:e,password:n});return o.$save({action:"login"})},postLogin:function(e){return new i.save({action:"loginApp"},{username:e.username})},token:function(e){var t=new i({username:e.username,accessToken:e.accessToken,requestToken:e.requestToken});return t.$save({action:"token"})},logout:function(){var e=this.getCurrentUser(),n=new i({username:e.username,accessToken:e.accessToken});return t.AuthParams={},localStorage.removeItem("AUTH_PARAMS"),n.$save({action:"logout"})},getCurrentUser:function(){var e=t.AuthParams;return e&&0!==Object.keys(e).length||(e=JSON.parse(localStorage.getItem("AUTH_PARAMS"))||void 0,e&&(n.defaults.headers.common.Authorization="Bearer "+e.accessToken)),e}}}),angular.module("qualitaCoreFrontend").service("AuthorizationService",function(e,t,n,o,i){var r=t(o.getBaseUrl()+"/authorization/:action",{action:"@action"});return{hasPermission:function(e,t){var n=t||i.getCurrentUser(),o=[];return n&&(o=n.permissions||[]),o.indexOf(e)>=0},principal:function(){return r.get({action:"principal"}).$promise},setupCredentials:function(t,o,i,r){var a={username:t,requestToken:o,accessToken:i};e.AuthParams=a,localStorage.setItem("AUTH_PARAMS",JSON.stringify(a)),n.defaults.headers.common.Authorization="Bearer "+i,this.principal().then(function(e){a.permissions=e.permisos,a.stamp=e.stamp,localStorage.setItem("AUTH_PARAMS",JSON.stringify(a)),r(a)})},cleanupCredentials:function(){localStorage.removeItem("AUTH_PARAMS")},authorize:function(e,t){var n=i.getCurrentUser();if(e===!0&&void 0===n)return this.enums.LOGIN_REQUIRED;if(e&&void 0!==n&&(void 0===t||0===t.length))return this.enums.AUTHORIZED;if(t){for(var o=!0,r=0;r<t.length&&(o=this.hasPermission(t[r],n),o!==!1);r++);return o?this.enums.AUTHORIZED:this.enums.NOT_AUTHORIZED}},enums:{LOGIN_REQUIRED:"loginRequired",NOT_AUTHORIZED:"notAuthorized",AUTHORIZED:"authorized"}}}),angular.module("qualitaCoreFrontend").factory("baseurl",function(e){return{getBaseUrl:function(){var t=window.location.hostname;return t===e.serverIp?"http://"+t+"/"+e.serverName+"/rest":"http://"+t+":8088/"+e.serverName+"/rest"},getPublicBaseUrl:function(){var t=window.location.hostname;return t===e.serverIp?"http://"+t+"/public/":"http://"+t+":8088/public/"}}}),angular.module("qualitaCoreFrontend").config(["schemaFormProvider","schemaFormDecoratorsProvider","sfPathProvider","flowFactoryProvider",function(e,t,n,o){o.defaults={method:"octet"};var i=function(t,o,i){if("object"===o.type&&"fileupload"===o.format){var r=e.stdFormObj(t,o,i);return r.key=i.path,r.type="fileupload",i.lookup[n.stringify(i.path)]=r,r}};e.defaults.object.unshift(i),t.addMapping("bootstrapDecorator","fileupload","views/directives/fileupload.html"),t.createDirective("fileupload","views/directives/fileupload.html")}]).factory("fileupload",function(){}),angular.module("qualitaCoreFrontend").factory("filterFactory",function(){var e=function(e,t){var i={_inner:{type:e}};return i._inner.filters=this&&this._inner?[this._inner,t]:t,i.or||"and"!==e||(i.or=n),i.value||(i.value=r),i.add||(i.add=o),i.paginate=a,i},t=function(t){return e.call(this,"and",t)},n=function(t){return e.call(this,"or",t)},o=function(e){return this.filters.push(e),this},i=function(e){return t([e])},r=function(){return this._inner},a=function(e,t){return this._inner.limit=e,this._inner.offset=t,this};return{and:t,or:n,add:o,single:i,value:r}}),angular.module("qualitaCoreFrontend").factory("formFactory",function(e,t,n,o){return{defaultForm:function(){return["*",{type:"submit",title:"Guardar",htmlClass:"pull-right"}]},defaultOptions:function(){return{formDefaults:{ngModelOptions:{updateOn:"blur"}},validationMessage:{302:"El campo es obligatorio"}}},defaultSubmit:function(i,r,a,l,s){if(r.$broadcast("schemaFormValidate"),o.isProcessing=!0,a.$valid){if(r.model)var c=l.create(r.model);else var c=l.create(s.model);l.save(c).then(function(){o.isProcessing=!1,e.path("/"+i)},function(){var e="Error al persistir la operación.";r.model.id||(e+="\n\nGuardando localmente, reintente más tarde."),n({message:e,classes:"alert-danger",position:"right"}),t.getItem(i).then(function(e){e=e||[],e.unshift(r.model),r.model.id||t.setItem(i,e)})})}}}}),angular.module("qualitaCoreFrontend").factory("HttpInterceptor",function(e,t,n,o,i){return{request:function(e){return"/login"!==t.path()&&(e.headers.Authorization="Bearer "+n.AuthParams.accessToken),e},requestError:function(n){return 401===n.status&&t.path("/login"),e.reject(n)},response:function(e){return e},responseError:function(o){var r=i.get("notify");if(401===o.status){if(o.data&&403===o.data.code)return r({message:o.data.error,classes:["alert-danger"]}),t.path("/"),e.reject(o);if("/login"===t.path())return e.reject(o);var a=e.defer(),l=i.get("AuthenticationService"),s=i.get("$http"),c=l.token(n.AuthParams);return c.then(function(e){n.AuthParams.accessToken=e.accessToken,localStorage.setItem("AUTH_PARAMS",JSON.stringify(n.AuthParams)),s.defaults.headers.common.Authorization="Bearer "+e.accessToken,l.postLogin(n.AuthParams).$promise.then(function(e){n.AuthParams.accesoSistema=e,s.defaults.headers.common["X-Access"]=n.AuthParams.accesoSistema.accesosSistema[0].locacion.id})}).then(a.resolve,a.reject),a.promise.then(function(){return o.config.headers.Authorization="Bearer "+n.AuthParams.accessToken,s(o.config)})}return e.reject(o)}}}),angular.module("qualitaCoreFrontend").factory("ReportTicketFactory",["$resource","baseurl",function(e,t){var n=e(t.getBaseUrl()+"/ticket/:reportID",{action:"@reportID"});return{ticket:function(e,t){var o=new n(t);return o.$save({reportID:e})},downloadURL:function(e,n){return console.log("downloadURL"),t.getBaseUrl()+"/generar/"+e+"/"+n}}}]),angular.module("qualitaCoreFrontend").factory("usuariosFactory",function(e,t,n){var o=e(n.getBaseUrl()+"/usuarios/:id",{id:"@id"},{update:{method:"PUT"}});return{all:function(e){return o.query(e)},get:function(e){return o.get({id:e})},getByUsername:function(e){var n={};return n.search=t.single({path:"username",equals:e}).value(),o.query(n)},create:function(e){return new o(e)},save:function(e){return e.id?e.$update():e.$save()},remove:function(e){return e.$remove()}}});
